...
def main():
    ...
    print(f'You have entered {guess.upper()} (value: {guess_value})')

    # Get best word possible from the drawn letters
    comp_word, comp_word_value = get_comp(letters)
    print(f'Optimal word possible: {comp_word.upper()} (value: {comp_word_value})')
...
def draw_samples(letter_bank):
    ...
    while len(lst) != 7:
        ...
        rem -= int(cnt)
        if rem > 0:
            print(f'You have drawn {len(lst)} letter{"s" if len(lst) > 1 else ""}, {(7-len(lst))} left')
            print(f'Current letters drawn: {lst}')
        else:
            print(f'You have drawn {len(lst)} letters')
